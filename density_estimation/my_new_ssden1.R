## Fit density model
my_ssden1 <- function(formula,type=NULL,data=list(),alpha=1.4,
                   ...
                   my_lambda, my_theta)
{
    ...
    ## Fit the model
    z <- my_mspdsty1(s,r,id.basis,cnt,int,prec,maxiter,alpha,
                     my_lambda, my_theta)
...
my_mspdsty1 <- function(s,r,id.basis,cnt,int,prec,maxiter,alpha,
                        my_lambda, my_theta)
{
    nq <- dim(r)[3]
    ## initialization
    ...
    ## theta adjustment
    # z <- my_sspdsty1(s,r.wk,r.wk[id.basis,],cnt,int.wk,prec,maxiter,alpha,
    #                  my_lambda, my_theta)
    # theta <- theta + z$theta
    # 
    # r.wk <- int.r.wk <- 0
...
my_sspdsty1 <- function(s,r,q,cnt,int,prec,maxiter,alpha,
                        my_lambda, my_theta)
{
    nxi <- dim(r)[2]
    nobs <- dim(r)[1]
    ...
    list(lambda=la,theta=theta,c=c,d=d,scal=scal,cv=jk1)
}
