## Fit gssanova model
my_gssanova <- function(formula,family,type=NULL,data=list(),weights,
                     ...
                     my_lam,my_the)
{
  ...
  if (nq==1) {
    r <- r[,,1]
    z <- my_sspngreg(family,s,r,r[id.basis,],y,wt,offset,alpha,nu.wk,random,
                     my_lam,my_the)
  }
  else z <- my_mspngreg(family,s,r,id.basis,y,wt,offset,alpha,nu.wk,random,skip.iter,
                        my_lam,my_the)
  tt = Sys.time() - tt1
  ...
  obj <- c(list(call=match.call(),family=family,mf=mf,terms=term,desc=desc,
                ...
                random=random,skip.iter=skip.iter,tt=tt),z)
...
my_sspngreg <- function(family,s,r,q,y,wt,offset,alpha,nu,random,
                        my_lam,my_the)
{
  nobs <- nrow(r)
  nxi <- ncol(r)
  ...
  c(list(theta=theta,ran.scal=ran.scal,c=c,d=d,b=b,nlambda=zz$est[1],
         zeta=zz$est[-1],nu=nu.wk),fit[-1],list(se.aux=se.aux),list(time = time))
}

## Fit Multiple Smoothing Parameter Non-Gaussian REGression
my_mspngreg <- function(family,s,r,id.basis,y,wt,offset,alpha,nu,random,skip.iter,
                        my_lam,my_the)
{
  nobs <- nrow(r)
  nxi <- ncol(r)
  ...
  ## theta adjustment
  z <- my_sspngreg(family,s,r.wk,r.wk[id.basis,],y,wt,offset,alpha,nu,random,
                   my_lam,my_the)
  ...
  c(list(theta=zz$est[1:nq],c=c,d=d,b=b,nlambda=nlambda,zeta=zz$est[-(1:nq)],nu=nu.wk),
    fit[-1],list(se.aux=se.aux),list(time = time))
}
...
cv.binomial <- function (y, eta, wt, hat, alpha) 
{
  ...
  list(score = lkhd + abs(alpha) * aux1 * aux2, varht = 1, 
       w = as.vector(wtt * w))
}
...
regaux <- function (s, r, q, nlambda, fit) 
{
  ...
  z <- .Fortran("regaux", as.double(fit$chol), as.integer(nn), 
                as.integer(fit$jpvt), as.integer(fit$rkv), drcr = as.double(t(cbind(s, 
                                                                                    r)) %*% r %*% vec), as.integer(rkq), sms = double(nnull^2), 
                as.integer(nnull), double(nn * nnull), PACKAGE = "gss")[c("drcr", 
                                                                          "sms")]
  ...
  wk1 <- z
  rkw <- attr(z, "rank")
  while (wk1[rkw, rkw] < wk1[1, 1] * sqrt(.Machine$double.eps)) rkw <- rkw - 
    1
  wk1[row(wk1) > col(wk1)] <- 0
  if (rkw < nnull + rkq) 
    wk1[(rkw + 1):(nnull + rkq), (rkw + 1):(nnull + rkq)] <- diag(0, 
                                                                  nnull + rkq - rkw)
  hfac <- wk1
